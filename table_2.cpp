...
To do so, we check that the solutions stored in table_2_morphisms.txt are valid

compilation : g++ -O3 -o table_2.o table_2.cpp
execution   : ./table_2.o
...
int main(int argc, char* argv[]){
    if(argc != 2){
        std::cout << "Usage 1 : ./table_2.cpp <p> (with p in {3,4,5,7,8,9,10,11,12,14,15,16,20,21,22})" << std::endl;
        return 1;
    }
    int p = std::stoi(argv[1]);

    // We read the morphism from table_2_morphisms.txt
    std::ifstream file("table_2_morphisms.txt");
    ...
    if(pr != p){
        std::cout << "Usage 2 : ./table_2.cpp <p> (with p in {3,4,5,7,8,9,10,11,12,14,15,16,20,21,22})" << std::endl;
        return 1;
    }
